...
async function login(request, response, next) {
    try {
        ...
        const user = await data_source_1.AppDataSource.getRepository(user_1.User)
        ...
        if (passwordHash != user.passwordHash) {
            const message = `Login denied.`;
            logger_1.logger.info(`${message} - user with ${email} has entered the wrong password.`);
            response.status(403).json({ message });
            return;
        }
        logger_1.logger.info(`User ${email} has now logged in.`);
